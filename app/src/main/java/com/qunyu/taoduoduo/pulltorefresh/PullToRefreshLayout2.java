...
//						LogUtil.ErrorLog(e);
                ...
                if (pullDownY > 0) {
                    // 刷新结果停留1秒
                    new RemainHandler(PullToRefreshLayout2.this)
    ...
    public void loadmoreFinish(int refreshResult) {
        ...
        if (pullUpY < 0) {
            // 刷新结果停留1秒
            new RemainHandler(this).sendEmptyMessageDelayed(0, 200);
        } else {
            changeState(DONE);
            hide();
        }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (Math.abs(ev.getX() - downX) > 150||Math.abs(ev.getY() - downY) < 150) {
    ...
    static class RemainHandler extends Handler {
        private WeakReference<PullToRefreshLayout2> mLayout;

        public RemainHandler(PullToRefreshLayout2 layout) {
            mLayout = new WeakReference<>(layout);
        }

        @Override
        public void handleMessage(Message msg) {
            PullToRefreshLayout2 layout = mLayout.get();
            if (null != layout) {
    ...
    static class UpdateHandler extends Handler {
        ...
        public UpdateHandler(PullToRefreshLayout2 layout) {
            mLayout = new WeakReference<>(layout);
        }
