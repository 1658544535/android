...
public class UserJoinInfoApiAdapter extends BaseAdapter {
    ...
    //列表展现的数据
    private ArrayList<UserJoinInfoBean.JoinUserListBean> mData;
     ...
     * @param data    列表展现的数据
     */
    public UserJoinInfoApiAdapter(Context context, ArrayList<UserJoinInfoBean.JoinUserListBean> data
    ) {
        mContext = context;
        mData = data;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //设置数据
        final UserJoinInfoBean.JoinUserListBean dataSet = mData.get(position);
        if (dataSet == null) {
            return null;
        }
        //获取该行数据
